...
<div class="col-md-12 filter-buttons-box">
	...
	<div class="col-md-7">
		<div class="sorters-box">
			...
			<?php
				echo '<div class="filter-button shopping-filter-button">';
				echo $this->Html->link(
	...
	<div class="col-md-5">
		<div class="filters-box">
			...
			<div class="filter-button shopping-filter-button">
			...
			<?php
				...
				echo '<div id="qtip-tags-arrows" class="shopping-qtip-arrows"></div>';
...
<script>
	...
	$(document).ready(function() {
		...
		$('#open-attraction-distance-filter-tooltip').qtip({
			...
			style: {
				classes: 'shopping-qtip qtip-distance',
				def: false
			},
		    position: {
		        my: 'top left',
		...
		$('#open-attraction-tag-filter-tooltip').qtip({
			...
			style: {
				classes: 'shopping-qtip qtip-tags',
				def: false
			},
		    position: {
		        my: 'top left',
	...
	function createAttractionLeftDiv(attraction) {
		var div = '<div class="attraction-info-box attraction-shopping-info-box">';
	...
	function createButtonBox(attraction) {
		...
		div += '<a href="'+attractionShow+'" class="attraction-button attraction-shopping-button">';
	...
	function createAttractionInfoBox(attraction) {
		...
		for (var i = 0; i < tags.length; i++) {
			div += '<span class="shopping-tag">' + tags[i]['Tag']['name_pt'] + '</span> ';
		}
		div += '</div>';
		div += '</div>';
	...
	function setDistanceFilter(distance) {
		...
		if (distance) {
			...
			var span = '<span class="chosen-filter shopping-filter attraction-distance-filter" data-distance="'+distance+'">';
			span += '<i class="fa fa-map-marker"></i> Até '+kmDistance+'km';
			span += '<span class="remove-filter remove-shopping-filter remove-distance-filter"><i class="fa fa-times"></i></span>';
			span += '</span>';
	...
	function addTagToFilter(id, name) {
		...
		if ($chosenTagFilters.find('.attraction-tag-filter[data-tag-id="'+id+'"]').length == 0) {
			var span = '<span class="chosen-filter shopping-filter attraction-tag-filter" data-tag-id="'+id+'">';
			span += '<i class="fa fa-tag"></i> ' + name;
			span += '<span class="remove-filter remove-shopping-filter remove-tag-filter"><i class="fa fa-times"></i></span>';
			span += '</span>';
